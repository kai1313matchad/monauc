<?php if(! defined ('BASEPATH')) exit ('Akses langsung tidak diperbolehkan');
	class Auth_
	{
		...
		{
			$query = $this->CI->db->get_where('mona_user', array('user_name'=>$username, 'user_pass'=>$password));
			if($query->num_rows() > 0)
			{
				$que1 = $this->CI->db->get_where('mona_user', array('user_name'=>$username, 'user_pass'=>$password));
				$usrdata = $que1->row();
				$ses = array(
					'log_id'=>uniqid(rand()),
					'user_id'=>$usrdata->USER_ID,
					'user_name'=>$usrdata->USER_NAME,
					'user_company'=>$usrdata->USER_COMPANY,
				);
